...
#pragma mark - Builder
+(NSDate *)buildDateForDay:(NSInteger)day andMonth:(NSInteger)month andYear:(NSInteger)year {
...
-(NSDate *)dateAtEndOfMonth {
    NSDateComponents *components = [[NSDate currentCalendar] components:componentFlags fromDate:self];
//    components.day = [Repeat daysInMonthByDate:self];
...
-(int)daysInMonthByDate:(NSDate *)date {
    return [self daysInMonth:date.month];
}
-(int)daysInMonth:(MonthNames)month {
    switch (month) {
        case January:
            return 31;
            break;
        case February: {
            NSString * yearString = [[NSDate date] stringWithFormat:@"yyyy"];
            int year = yearString.intValue;
            if (year % 4 == 0) {
                NSString * yearLastStrings = [yearString stringByReplacingCharactersInRange:NSMakeRange(0, 2) withString:@""];
                if (yearLastStrings.intValue == 00) {
                    if (year % 400 == 0) {
                        return 29;
                    } else {
                        return 28;
